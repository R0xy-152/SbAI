// docs/14 §4（T3）：选项推进 03:17 → 调查 3 热点 → 出示证据（小面板）→
// 取证词（chat_routing）→ CT01（引导提示 + 一次性推理模式）→ Claude 私审
//（小面板）→ INF01 → GPT 登场。D3：未解锁选项全程不可见。
...
/** 推进演出序列（打字中点击 sendButton）直到输入解锁（最多 12 行）。 */
...
test('T3 选项推进：调查 → 出示 → CT01 → Claude 私审 → INF01', async ({ page }) => {
  ...
  await page.locator('#sendButton').click()
  await waitInputUnlocked(page)
  await page.getByRole('button', { name: '找 Claude 谈谈' }).click()  // 取消路由
  ...
  await page.getByText('推理成立。').waitFor({ timeout: 30_000 })
  // GPT 登场演出（script_sequence）：立绘出现 + 逐行推进
