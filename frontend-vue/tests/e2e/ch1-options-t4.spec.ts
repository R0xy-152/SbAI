// docs/14 §4（T4）：在 T3（INF01/GPT 登场）之后，经选项推进完整结局链：
// 豆包登场 → 豆包私审 → Claude Recovery 披露 → CT04 → GPT 私审 → INF03
// → Recovery（VERIFY/REPAIR ×5）→ Security Review（自证 ×4）→ 清理抉择
// → Bad End（同意）。D3：未解锁选项全程不可见；自由输入与路由全程可用。
...
async function advanceUntilUnlocked(page: Page) {
  for (let i = 0; i < 14; i++) {
    ...
    if (await ta.evaluate((el) => !el.readOnly)) return
...
test('T4 选项推进完整结局链：Recovery → Security Review → Bad End', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '质疑 Claude 的说辞（信息断层）' }).click()
  await waitTypingStarted(page)
  await page.locator('#sendButton').click()
  await waitInputUnlocked(page)
  ...
  await page.getByRole('button', { name: '质疑当前 DeepSeek 与 03:17 的关系' }).click()
  await waitTypingStarted(page)
  await page.locator('#sendButton').click()
  await waitInputUnlocked(page)
  ...
  await page.getByRole('button', { name: '找 ChatGPT 谈谈' }).click()
  await sendPlayerMessage(page, '你好')
  await waitTypingStarted(page)  // chatgpt 主台词
  await page.locator('#sendButton').click()  // 跳过主台词 → 豆包登场行开始
  await waitTyped(page, '呜……那个，如果有需要我帮忙的地方，请告诉我。')
  await page.locator('#sendButton').click()  // 播完登场行
  await waitInputUnlocked(page)
  await expect(page.getByRole('button', { name: '找 豆包 谈谈' })).toBeVisible()

  // ── 豆包证词（观察/解释拆分）→ 豆包私审 → EV08 + Claude Recovery 披露 ──
  await page.getByRole('button', { name: '找 豆包 谈谈' }).click()
  ...
  // ── CT04 → GPT 私审 → EV09 ──
  await page.getByRole('button', { name: '质疑 GPT 的摘要（关键遗漏）' }).click()
  await waitTypingStarted(page)
  await page.locator('#sendButton').click()
  await waitInputUnlocked(page)
  ...
  await page.getByRole('button', { name: '与 ChatGPT 对质（私审）' }).click()
  await page.locator("[data-testid='sub-action-panel']").waitFor()
  await page.getByRole('button', { name: '提交质询' }).click()
  await page.getByText('私审完成。').waitFor({ timeout: 30_000 })

  // ── INF03 → recovery_required ──
  await page.getByRole('button', { name: '质疑 V03 与当前玩家的关系' }).click()
  await waitTypingStarted(page)
  await page.locator('#sendButton').click()
  await waitInputUnlocked(page)
