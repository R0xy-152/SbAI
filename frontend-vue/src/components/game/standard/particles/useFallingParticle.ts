/*
  Adapted from LingChat (AGPL-3.0): src/components/game/standard/particles/hooks/useFallingParticle.ts（含 types/falling.ts）
  Modification（docs/15 §3）：类型内联合并；其余原样适配。
*/
import { ref, onMounted, onUnmounted, watch, nextTick, type Ref } from 'vue'

// 内联类型（docs/15 §3：合并 LingChat 的 types/falling.ts，避免单接口拆文件）
...
export function useFallingParticle<T extends FallingParticle>(
  ...
  const { baseCount, createParticle, generateKeyframes } = options
  ...
  const createParticles = (count: number): void => {
    for (let i = 0; i < count; i++) {
      ...
      const particle = createParticle(id) as T
      particle.id = id
