<!--
  Adapted from LingChat (AGPL-3.0): src/components/game/standard/particles/StarField.vue
  Modification（docs/15 §6.3）：原样适配；新增无 2d context 时静默降级。
-->
<template>
...
// @ts-nocheck —— LingChat 原文件为纯 JS；保持原样适配（docs/15 §3），
// 不做逐参数类型标注，避免无谓改动。
...
onMounted(() => {
  if (canvasRef.value && props.enabled) {
    try {
      ...
      // 无 2d context（测试环境）时静默降级：不渲染、不报错
