<!--
  Adapted from LingChat (AGPL-3.0): src/components/game/standard/particles/Rain.vue（含 hooks/useRain.ts + config/rain.ts）
  Modification（docs/15 §6.3）：内联合并雨滴工厂；其余原样适配。
-->
<template>
...
// docs/15 §6.3：合并 LingChat 的 hooks/useRain.ts 与 config/rain.ts（原路径
// 见 THIRD_PARTY_LICENSES.md），避免为一个小工厂拆两个文件。
interface Drop {
  x: number
  y: number
  speed: number
...
function createDrop(W: number, H: number, intensity = 1): Drop {
  return {
    x: Math.random() * W,
    y: Math.random() * H - 100,
