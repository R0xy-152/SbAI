<!--
  Adapted from LingChat (AGPL-3.0): src/components/effects/CursorEffects.vue
  Modification（docs/15 §5.1）：settings store 换为本项目 settings store
  （globalMouseTrailEnabled / clickAnimationEnabled，默认开启）；弹窗抑制
  选择器适配为本项目 .gal-modal-mask；其余渲染逻辑（双缓冲 + 脏矩形 +
  globalCompositeOperation lighter）原样保留。
-->
<template>
...
class MouseSpark {
  ...
  bindHandlers() {
    ...
    this.onMouseDown = (e: MouseEvent) => {
      ...
      if ((e.target as HTMLElement)?.closest('.gal-modal-mask, .blur-overlay')) return
      this.isDown = true
