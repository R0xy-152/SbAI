<!--
  Adapted from LingChat (AGPL-3.0): src/components/views/LoadingTransition.vue
  Modification（docs/15 §7）：
  - i18n 文案改为内联中文；随机提示语换为本项目 docs 派生的提示；
  - 进度事件源从 LingChat eventQueue 改为父组件 ready prop（opening 数据就绪）；
  - 最短展示 2.4s（Web 版缩短，LingChat 为 5s）/ 最长 12s 强制完成；
  - WebAudio 合成音效（tick/chime/pop/unveil）保留，autoplay 失败静默；
  - 其余视觉（黑幕、赛博网格、扫描线、四角、粒子、进度圈、猫爪遮罩揭幕）保留。
-->
<template>
  <!-- 全屏加载过渡层 -->
  <div class="loading-transition-root">
    ...
    >
      ...
      <!-- 四角装饰 -->
      <div class="absolute top-4 left-4 w-8 h-8 border-l-2 border-t-2 border-teal-500/30 pointer-events-none"></div>
      <div class="absolute top-4 right-4 w-8 h-8 border-r-2 border-t-2 border-teal-500/30 pointer-events-none"></div>
      <div class="absolute bottom-4 left-4 w-8 h-8 border-l-2 border-b-2 border-teal-500/30 pointer-events-none"></div>
      <div class="absolute bottom-4 right-4 w-8 h-8 border-r-2 border-b-2 border-teal-500/30 pointer-events-none"></div>
      ...
      <div class="relative flex flex-col items-center justify-center my-auto z-10">
        ...
        <div class="relative w-64 h-64 flex items-center justify-center">
          <svg
            ...
            viewBox="0 0 100 100"
          >
            <circle cx="50" cy="50" r="44" stroke="rgba(45, 212, 191, 0.08)" stroke-width="2" fill="none" />
            <circle
              cx="50"
              cy="50"
              r="44"
            ...
            />
            <circle cx="50" cy="50" r="48" stroke="rgba(45, 212, 191, 0.15)" stroke-width="1" fill="none" />
            <circle cx="50" cy="50" r="51" stroke="rgba(45, 212, 191, 0.10)" stroke-width="0.8" fill="none" />
            <defs>
        ...
        <div class="mt-8 flex flex-col items-center space-y-2">
          ...
          <div class="w-[40vh] text-center text-sm font-mono text-cyan-400/70 transition-all duration-300">
            {{ currentStatusText }}
          </div>
          ...
          <div class="w-[60vh] flex items-center space-y-2 mt-4">
            ...
            <div class="absolute -right-12 justify-center items-center text-sm font-mono text-cyan-300/80">
              <span>{{ Math.floor(progress) }}%</span>
            </div>
          </div>

          <div class="w-[40vh] flex text-center justify-center gap-12 text-sm font-mono text-cyan-300/80">
...
// docs/15 §7：ready=true（opening 数据就绪）后 1s 内冲满进度。
...
// ============================================================
//  Web Audio 音效合成器（无采样文件；autoplay 失败静默）
// ============================================================
const NekoSynth = {
...
// ============================================================
//  加载状态台词 / 随机小贴士（docs/15 §7：本项目 docs 派生的中文文案）
// ============================================================
const statusTexts = [
  '正在连接后端…',
  '初始化角色运行时…',
...
function updateStatusText(p: number) {
  ...
  for (const t of thresholds) {
    if (p >= t && !triggeredThresholds.has(t)) {
      ...
      const idx = Math.min(Math.floor(t / 20), statusTexts.length - 1)
      currentStatusText.value = statusTexts[idx] ?? statusTexts[0]!
      return
    }
  }
}
...
// ============================================================
//  进度条模拟算法（docs/15 §7：MIN 2.4s / MAX 12s / ready 加速）
// ============================================================
...
function computeTarget(elapsed: number): number {
  ...
  if (props.ready && eventDetectedTime === 0) {
...
function handleTransitionSequence() {
  ...
  // 等 1.0s 让用户看到 "Connection Established" 后开始转场
  setTimeout(() => {
...
watch(
  () => props.ready,
  () => {
    // ready 到达时 eventDetectedTime 在 computeTarget 内登记（下一 tick 生效）
  },
)
