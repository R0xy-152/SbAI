...
// 由 SavePanel/LoadPanel 传入 slot 元数据与动作回调；组件只负责展示与
// 触发，不持有 Save 逻辑（改动集中在父级 store/View）。
...
<template>
  ...
  >
    <div class="flex items-baseline gap-2">
      ...
      <span
        v-else
        class="text-xs text-[#d7effa]/40"
      >
        空存档位
      </span>
      <button
        v-if="save && mode === 'load'"
        class="rounded border border-red-300/30 px-1.5 py-0.5 text-[10px] text-red-300/80 transition-colors hover:bg-red-500/20 disabled:opacity-40"
        :disabled="busy"
        :title="'删除此存档'"
