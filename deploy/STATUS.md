...
- 已部署 commit：f0d4c4a（backend 镜像首次构建）+ e5c9049（backend 镜像第二次构建：评测 v2，无运行时行为变化）+ 仓库 HEAD 同步（9085c0a nginx /ops 转发、cde713f compose 环境转发、5413191 产品案例文档）；含 4e85c86 全部内容（AI 对话人机感改造 + save v2 迁移 + 对开发者的话 + 二维码登录 + 画像召回修复 + HTTPS 加固）**及新增**：运营监控与反馈分析闭环（docs/21——事件埋点/漏斗看板/AI 指标/反馈分类+人工抽检，内部入口 https://sbai.xin/ops，GAL_OPS_TOKEN 门禁）
