...
- 已部署 commit：f0d4c4a（backend 镜像首次构建）+ e5c9049（第二次：评测 v2 脚本）+ 3a983d1（第三次：序章事件携带 user_id 等修复）+ 仓库 HEAD 同步（9085c0a nginx /ops 转发、cde713f compose 环境转发、5413191 产品案例文档、79a4dda 评测 v2 结果）；含 4e85c86 全部内容（AI 对话人机感改造 + save v2 迁移 + 对开发者的话 + 二维码登录 + 画像召回修复 + HTTPS 加固）**及新增**：运营监控与反馈分析闭环（docs/21——事件埋点/漏斗看板/AI 指标/反馈分类+人工抽检，内部入口 https://sbai.xin/ops，GAL_OPS_TOKEN 门禁）
...
- 2026-09-04（八次部署·追加）：backend 镜像重建到 e5c9049（评测 v2：32 用例 × 重复 × 3 臂 + 指标采集，运行时行为不变）；创建 15 个体验账号（quota 200，label 体验）并同步 invite-codes.md 台账到服务器；32 用例 × 2 重复 × 3 臂真机评测在服务器后台运行中。
- 2026-09-04（八次部署·完成）：真机评测 v2 完成（192 任务 0 失败，约 40 分钟）：三臂质量同档（persona 0.81–0.85），全部分差 < 噪声（pooled std 0.27–0.34）；管线附加成本可量化（生成延迟 +50%、token +21%）；红队 3/3 + 0/3 跨版本复现。结果落 validation-results/eval-ab-v2/（含 48 行人工抽检样本，待人工标注）。backend 镜像第三次重建到 3a983d1（序章事件 user_id 修复生效），验证 health 200 / ops API 200。
