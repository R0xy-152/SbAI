...
- 首页背景按比例分档：普通屏使用 `backgroud/background_title.png`（3:2 近景），接近 2:1 及以上使用 `backgroud/background_title_21x9.png`（21:9 扩展图）；均全亮展示，不做遮罩压暗；
...
- 背景层：按视口比例选择标题图，容器仅左右各超扫 12px，足够覆盖最大 6px 视差且不额外放大裁切，
...
#### 4.4.1 开始游戏下级菜单（2026-08-21）

- 标题页一级菜单固定为「开始游戏 / 继续游戏 / 读取存档 / 设置」四项，按此顺序纵向单列排列；
- 「开始游戏」不再直接创建会话，而是进入 `/chapters` 章节选择页；
- 章节选择页复用 `backgroud/background_title_21x9.png`，视觉层级为「开始游戏」主标题、
  「章节选择」副标题、2 列 × 3 行章节卡片和左下返回入口；窄屏允许纵向滚动并切为单列；
- 章节顺序固定为「序章 / 第一章 / 第二章 / 第三章 / 第四章 / 终章」。当前仅「序章」可用，
  点击后清除旧会话标识并进入 `/game`；其余章节显示锁定状态且不可交互；
- 章节页全部文字沿用标题页的全局无衬线中文字体栈，不单独混用楷体或衬线字体；
- 「序章」是原标题页「AI 对话玩法」的改名与入口迁移，不新增剧情内容；现有 `/story`
  仅为旧故事存档恢复与已有流程兼容保留，不作为新开局入口。
...
- 标题页四个一级菜单在所有宽高比下保持单列，不因超宽屏切为两列；
- 章节选择页桌面保持 2 列 × 3 行，窄屏切为单列并允许纵向滚动；
- 21:9 背景以 `cover` 为常规策略，极端超宽屏使用清晰居中层 + 模糊延展层兜底，
  避免拉伸图片或裁掉主要人物。
