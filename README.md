...
**评测 v2**（2026-09-04，32 用例 × 2 重复 × 3 臂，n=64/臂）：三臂质量同档（persona 0.81–0.85，全维度 0.70–0.92），**所有臂间分差 < 噪声**（pooled std 0.27–0.34）——管线强约束不以质量为代价；管线附加成本可量化（生成延迟 +50%、token +21%）；同臂两次重复平均分差 0.25（评审噪声量化）；人工抽检 48 行评审-人工一致率 **87.5%**。逐行数据见 `validation-results/eval-ab-v2/`。
