...
**当前阶段**：进入 **docs/13（LingChat 前端源码迁移、开始界面与存档系统落地方案）**，前端从旧静态 UI 迁移到 Vue 3，并实现 Title Screen / Save-Load / Auto Save。Task 0-4 已完成（Task 0/1 PASS_WITH_LIMITATION，Task 2/3/4 PASS），Task 5-9 待执行。验证记录在 `validation-results/TV-xx/result.md` 与 `validation-results/docs13-task*/result.md`。
...
**Docker**：仓库根 `docker-compose.yml` 已存在，当前仅定义 `frontend-vue` 服务（8080:80，nginx 托管 dist + /api 反代）；`backend` / `postgres` 服务待 docs/13 Task 6 加入。本机无 Docker 时用 `npm run dev` + backend:8000 等价验证（docs/13 Task 1 采用此路径）。
